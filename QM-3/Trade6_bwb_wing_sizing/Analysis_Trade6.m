...
% This script generates plots to analyze Trade 6 for QM-3

%% Read data
n1 = 2420;
n2 = 4;

[rc, st, ~] = extractData(n1, "/Users/michaelchen/Documents/M-Fly/2023/QM-3/PADA_Trade6", true, false);
rc = cell2mat(rc);
st = cell2mat(st);
...
parameters = readtable("/Users/michaelchen/Documents/M-Fly/2023/QM-3/PADA_Trade6/QM-3_Trade6_inches.csv");
parametersMeters = readtable("/Users/michaelchen/Documents/M-Fly/2023/QM-3/PADA_Trade6/QM-3_Trade6_meters.csv");
...
%% Plot L/D with AoA < 8, statically stable, and with static margin between 5-15%
AoA = [rc.alpha];
Cma = [st.Cma];
...
LD(AoA >= 8 | Cma >= 0 | staticMargin < 5 | staticMargin > 15) = NaN;

plot4D(parameters.sweepAngle, parameters.bref, parameters.taperRatio, parameters.transitionWidth, LD, n1, n2, ...
    "Sweep angle", "Wing span", "Taper ratio", "Transition width", "L/D");

%% Plot sref with AoA < 8, statically stable, with static margin between 5-15%, and sref < 162 in^2 (as small as QM-3D)
AoA = [rc.alpha];
Cma = [st.Cma];
...
sref(AoA >= 8 | Cma >= 0 | staticMargin < 5 | staticMargin > 15 | sref > 162) = NaN;
...
%% Plot static margin with AoA < 8, statically stable, with static margin between 5-15%, and sref < 162 in^2 (as small as QM-3D)
AoA = [rc.alpha];
Cma = [st.Cma];
...
staticMargin(AoA >= 8 | Cma >= 0 | staticMargin < 5 | staticMargin > 15 | sref > 162) = NaN;

plot4D(parameters.sweepAngle, parameters.bref, parameters.taperRatio, parameters.transitionWidth, staticMargin, n1, n2, ...
    "Sweep angle", "Wing span", "Taper ratio", "Transition width", "Static margin");

%% Plot L/D with AoA < 8, statically stable, with static margin between 5-15%, and sref < 162 in^2 (as small as QM-3D)
AoA = [rc.alpha];
Cma = [st.Cma];
...
LD(AoA >= 8 | Cma >= 0 | staticMargin < 5 | staticMargin > 15 | sref > 162) = NaN;
...
taper = parameters.taperRatio';

LD(AoA >= 8 | Cma >= 0 | staticMargin < 5 | staticMargin > 15 | sref > 162 | taper <= 0.2) = NaN;
